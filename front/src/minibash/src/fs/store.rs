...
pub trait StoreExt: Store {
    fn entries<P: AsRef<Path>>(&self, path: P) -> io::Result<Entries> {
        <Self as Store>::entries_path(self, &crate::fs::index::normalize_path(path.as_ref()))
    }

    fn open<P: AsRef<Path>>(&self, path: P) -> io::Result<Self::File> {
        <Self as Store>::open_path(self, &crate::fs::index::normalize_path(path.as_ref()))
    }
}
    ...
    I: Iterator<Item = io::Result<Entry>>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            for result in self.inner[0].by_ref() {
                match result {
